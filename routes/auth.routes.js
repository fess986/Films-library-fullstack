...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (!errors.isEmpty()) {
        // проверяем ли есть что в массиве ошибок
        return res.status(400).json({
          // возвращаем статус 400 - ошибка
      ...
      if (candidate) {
        ...
        }) // return в начале используем для того чтобы скрипт остановился.
      }
  ...
  async (req, res) => {
    try {
      ...
      if (!user) {
        ...
        return res
          .status(400)
          .json({ message: 'Пользователь с таким email не найден' })
      }
      ...
      if (!isMatch) {
        return res.status(400).json({
          message: 'Неверный пароль для данного пользователя',
        })
      }
      ...
      console.log(error)
      res.status(500).json({
        message: 'Авторизация пользователя не удалась',
      })
    }
  }
)
