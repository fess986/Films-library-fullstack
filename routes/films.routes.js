...
router.get(ApiRoutes.GET_FILMS, isAuth, async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).json({message: 'Что-то пошло не так при получении фильмов с сервера'}) 
    // console.log(error);
}
});
...
router.post(ApiRoutes.SET_FILMS, isAuth, async (req, res) => {
  try {
    ...
    console.log(error);
    res.status(500).json({message: 'Что-то пошло не так при загрузке фильмов на сервер'}) 
}
});
