...
describe('reviewsSlice reducer', () => {
  it('should return the initial state', () => {
    expect(reviewsSlice.reducer(undefined, { type: 'undefined' })).toEqual(initialReviewsState);
  });

  it('should handle setIsReviewsLoaded', () => {
    const newState = reviewsSlice.reducer(initialReviewsState, setIsReviewsLoaded(true));
    expect(newState.isReviewsLoaded).toBe(true);
  });

  it('should handle setIsReviewSending', () => {
    const newState = reviewsSlice.reducer(initialReviewsState, setIsReviewSending(true));
    expect(newState.isReviewSending).toBe(true);
  });

  it('should handle setReviewsList', () => {
    const reviews = [createFakeReview(), createFakeReview()];
    const newState = reviewsSlice.reducer(initialReviewsState, setReviewsList(reviews as Review[]));
    expect(newState.reviewsList).toEqual(reviews);
  });

  it('should handle setUserReviewsList', () => {
    const userReviews = [createFakeReview(), createFakeReview(), createFakeReview()];
    const newState = reviewsSlice.reducer(initialReviewsState, setUserReviewsList(userReviews));
