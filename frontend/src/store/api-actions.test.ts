...
describe('fetchFilmsDB thunk', () => {
  ...
  it('dispatches actions correctly on success', async () => {
    ...
    mockAxios.onGet(`${baseURL}${ApiRoutes.FILMS}${ApiRoutes.GET_FILMS}`).reply(200, mockFilms); // мокаем запрос на получение фильмов. Запрос должен в точности совпадать с реальным
  ...
  it('dispatches actions correctly on failure', async () => {
    mockAxios.onGet(`${baseURL}${ApiRoutes.FILMS}${ApiRoutes.GET_FILMS}`).reply(500);
