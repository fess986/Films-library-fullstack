...
import { appSlice, initialAppState, setFilmsShownCount, setActiveGenre, setIsActiveFilmLoaded, setIsFilmsLoaded, setIsSimilarFilmsLoaded, setIsDataLoading, setError, resetFilmsShownCount } from './appSlice';

describe('appSlice reducer', () => {

  // передаём неизвестное действие, соответственно ничего не должно произойти и поменяться
  it('should return the initial state', () => {
    expect(appSlice.reducer(undefined, { type: 'someUnknownAction' })).toEqual(initialAppState);
  });
  ...
  it('should handle setIsActiveFilmLoaded', () => {
    const newState = appSlice.reducer(initialAppState, setIsActiveFilmLoaded(true));
  ...
  it('should handle setIsSimilarFilmsLoaded', () => {
    const newState = appSlice.reducer(initialAppState, setIsSimilarFilmsLoaded(true));
  ...
  it('should handle setError', () => {
    const newState = appSlice.reducer(initialAppState, setError('Network error'));
