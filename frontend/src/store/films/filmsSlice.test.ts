...
import { filmsSlice, initialFilmsState, setActiveFilm, setFilmList, setSimilarFilmList } from './filmsSlice';
...
describe('filmsSlice reducer', () => {
  it('should return the initial state', () => {
    expect(filmsSlice.reducer(void 0, { type: 'undefined' })).toEqual(initialFilmsState);
  });
  ...
  it('should handle setSimilarFilmList', () => {
    const films = [film1, film2, film3];
    const newState = filmsSlice.reducer(initialFilmsState, setSimilarFilmList(films));
  ...
  it('should handle fetchFilmsDB.rejected', () => {
    ...
    expect(browserHistory.push).toHaveBeenCalledWith(`${AppRoutes.ROOT}${AppRoutes.SIGN_IN}`);  // проверяем, запускалась ли функция push с аргументом `${AppRoutes.ROOT}${AppRoutes.SIGN_IN}`
  });
});
